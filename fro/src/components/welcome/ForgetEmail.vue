...
const startReset = () => {
  formRef.value.validate((isValid) => {
    if(isValid) {
      post('/api/auth/start-reset-email', {
        email: form.email,
        code: form.code
      }, () => {
        active.value++
    ...
    <transition name="el-fade-in-linear" mode="out-in">
      <div style="text-align: center;margin: 0 20px;height: 100%" v-if="active === 0">
        ...
        <div style="margin-top: 20px">
          <el-button @click="router.push('/welcome/forgetphone')"  style="width: 270px;" type="warning" plain>使用手机号验证</el-button>
        </div>
      </div>
    </transition>
    <transition name="el-fade-in-linear" mode="out-in">
